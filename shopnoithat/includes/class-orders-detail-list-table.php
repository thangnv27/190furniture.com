...
//            echo '<a href="' . $printLink . '" target="_blank" class="button" onclick="window.open(\'' . $printLink . '\', \'windowname1\', \'width=420,height=600,top=0,left=0,scrollbars=1\'); return false;">In hóa đơn</a>';
    ...
    function get_columns() {
        return $columns = array(
            ...
            'col_orders_name' => __('Name', SHORT_NAME),
            'col_orders_price' => __('Price', SHORT_NAME),
            'col_orders_quantity' => __('Quantity', SHORT_NAME),
            'col_orders_discount' => __('Discount', SHORT_NAME),
            'col_orders_amount' => __('Amount', SHORT_NAME),
            'col_orders_notes' => __('Notes', SHORT_NAME),
        );
    }
    ...
    function prepare_items() {
        ...
        $before_vat = number_format($ordersRow->before_vat, 0, ',', '.');
        $vat = number_format($ordersRow->before_vat * 0.1, 0, ',', '.');
        ...
        <h3>Tổng cộng trước thuế: <font color='red'>$before_vat</font> đ</h3>
        <h3>Thuế 10%: <font color='red'>{$vat}</font> đ</h3>
                ...
                foreach ($columns as $column_name => $column_display_name) {
                    ...
                    switch ($column_name) {
                        ...
                        case "col_orders_amount": echo '<td ' . $attributes . '>' . number_format($rec->amount, 0, ',', '.') . ' đ</td>';
                            break;
                        case "col_orders_notes": echo '<td ' . $attributes . '>Màu: ' . $rec->color . '</td>';
                            break;
                    }
                }
